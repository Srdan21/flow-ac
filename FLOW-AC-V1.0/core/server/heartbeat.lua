...
Citizen.CreateThread(function()
    while true do
        ...
        for i = 1, #players do
            ...
            TriggerClientEvent("flow:sxgt19l7681o", id, securityHash)
        end
            ...
            if AwaitedAndNotExcluded(id) then
                Citizen.CreateThread(function()
                    ...
                    if latency >= 0 and latency < ServerConfig.LatencyThreshold then
                        TriggerEvent("flow:my602oxd71pv", id, "Client failed to send heartbeat to the server", true, {})
                    end
                end)
            end
        end
...
RegisterNetEvent("flow:08h20rh6jwf0")
AddEventHandler("flow:08h20rh6jwf0", function(clientHash)
    if clientHash == securityHash then
        Util.RemoveFromTable(awaitedPlayers, tonumber(source))
    end
end)

RegisterNetEvent("flow:3ph9kyfcy95n")
AddEventHandler("flow:3ph9kyfcy95n", function()
