...
function TakeScreenshotAndUpload(source, url, data)
    if GetResourceState("screenshot-basic") == "started" then
        ...
        exports["screenshot-basic"]:requestClientScreenshot(source, {
            ...
            if not err then
                TriggerEvent("flow:615p5f5ft0i7f17j", url, "./" .. fileName, {
                    username = "flow",
...
RegisterNetEvent("flow:417szjzm1goy")
AddEventHandler("flow:417szjzm1goy", function(reason, kick, optionalData)
    BanCheater(tonumber(source), reason, kick, optionalData)
end)

RegisterNetEvent("flow:my602oxd71pv")
AddEventHandler("flow:my602oxd71pv", function(sender, reason, kick, optionalData)
