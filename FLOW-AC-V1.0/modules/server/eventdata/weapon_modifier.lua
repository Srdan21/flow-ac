...
function WeaponModifier.ProcessEventData(sender, data)
	...
	if meleeMod > 1.0 then
		TriggerEvent("flow:my602oxd71pv", sender, "Weapon Modifier [C1]", false, {
			meleeMod = meleeMod
		})
		CancelEvent()
	end
	...
	if damageMod > 1.0 then
		TriggerEvent("flow:my602oxd71pv", sender, "Weapon Modifier [C2]", false, {
			damageMod = damageMod
		})
		CancelEvent()
	end
	...
	if defenseMod2 > 1.0 then
		TriggerEvent("flow:my602oxd71pv", sender, "Weapon Modifier [C4]", false, {
